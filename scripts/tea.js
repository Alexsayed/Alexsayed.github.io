...
function radioFunction(){
	for(var i =0; i<allLis.length; i++){
		...
		if(whichRadio[1].checked === true){
			...
			if(aName[i].textContent.includes("Букет") || 
				aName[i].textContent.includes("черный") ||
				aName[i].textContent.includes("завтрак") ||
				aName[i].textContent.includes("грей") ||
				aName[i].textContent.includes("цейлон") || 
				aName[i].textContent.includes("Санрайз") ||
				aName[i].textContent.includes("Бергамот") || 
				aName[i].textContent.includes("чебрец") ) {	
				allLis[i].classList.add("black");
				// When the fruit radiobutton is selected then count the number of the items and display it
				var varBlack = document.querySelectorAll(".black");
				countOfItems.textContent = varBlack.length +" Результатов";
				
				// alert("show")
		 	}
		 }
		if(whichRadio[1].checked === false){
			allLis[i].classList.remove("black");		
		}
		...
		if(whichRadio[2].checked === true){
			...
			itemsToView[0].classList.remove("selected");
			itemsToView[1].classList.remove("selected");
			if(aName[i].textContent.includes("зеленый") ) {	
				allLis[i].classList.add("green");
				// Count of items to dispay
				var varGreen = document.querySelectorAll(".green"); 
				countOfItems.textContent = varGreen.length +" Результатов";
		 	}
		 }
		if(whichRadio[2].checked === false){
			allLis[i].classList.remove("green");		
		}
		...
		if(whichRadio[3].checked === true){
			...
			itemsToView[0].classList.remove("selected"); 
			itemsToView[1].classList.remove("selected");
			if(aName[i].textContent.includes("Каркаде") ) {	
				allLis[i].classList.add("herb");
				// Count of items to dispay
				var varHerb = document.querySelectorAll(".herb");
				countOfItems.textContent = varHerb.length +" Результатов";
		 	}
		 }
		if(whichRadio[3].checked === false){
			allLis[i].classList.remove("herb");		
		}
	}		
}
			...
			for( var j=0; j<allLis.length; j++){
				...
				if(itemsToView[0].classList.contains("selected")){
					...
					if(allLis[j].className !== "itemLis bestMatch allLis"){
						allLis[j].classList.remove("black"); 
						allLis[j].classList.remove("green");
						allLis[j].classList.remove("herb");
